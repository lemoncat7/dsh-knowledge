...
可写挂载必须提供受服务端范围约束的 `knowledge_write` 工具。更新已有知识使用当前会话搜索得到的签名 handle，新建知识使用当前可写挂载的精确名称或 ID；工具不得让模型指定本地或远程位置。写入结果仍由挂载的审核/直写模式以及服务端去重、合并和冲突保护决定。回答后的自动提取作为未主动调用工具时的兜底。
