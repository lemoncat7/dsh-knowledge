...
test('management console serves a secured same-origin application', async (t) => {
  ...
  assert.doesNotMatch(effects, /ResizeObserver|feDisplacementMap|knowledge-glass-filter|knowledgeGlass="(?:fallback|svg)"/)
  assert.match(effects, /\["\.sidebar","\.main"\]\.join\(","\)/)
  ...
  assert.match(styles, /\.sidebar\.knowledge-glass-surface,[\s\S]*?\.main\.knowledge-glass-surface \{[\s\S]*?background: var\(--workspace-glass-surface\);[\s\S]*?box-shadow: var\(--glass-shadow\);/)
  assert.match(styles, /\.note-workspace \{[\s\S]*?background: transparent;[\s\S]*?box-shadow: none;/)
  assert.match(styles, /\.topbar\.knowledge-glass-surface,[\s\S]*?background: transparent;[\s\S]*?backdrop-filter: none;/)
  assert.match(styles, /\.note-tree-panel\.knowledge-glass-surface,[\s\S]*?background: transparent;[\s\S]*?backdrop-filter: none;/)
