...
test('management console serves a secured same-origin application', async (t) => {
  ...
  assert.match(styles, /:root\[data-dsh-embed-mode="embedded"\] \{[\s\S]*?--workspace-glass-surface: rgba\(255, 255, 255, 0\.08\);[\s\S]*?--glass-filter: saturate\(\.65\) contrast\(1\.015\) blur\(24px\);/)
  assert.match(styles, /:root\[data-dsh-embed-mode="embedded"\]\[data-color-scheme="dark"\] \{[\s\S]*?--workspace-glass-surface: rgb\(16 23 25 \/ 18%\);[\s\S]*?--glass-filter: saturate\(\.55\) contrast\(1\.02\) blur\(24px\);/)
