...
test('remote provider interoperates with the authenticated local API', async (t) => {
  ...
  assert.deepEqual(health, { ok: true, service: 'dsh-knowledge', schemaVersion: 10 })
  ...
  const note = await local.notes.createDocument('Central service source', null, '# Private source note\n\nDo not expose this body through metadata search.')
  const noteMatches = await remote.searchNotes('Central service', 10)
  assert.deepEqual(noteMatches.map(item => item.id), [note.id])
  assert.equal(Object.hasOwn(noteMatches[0], 'content'), false)
