...
test('remote provider interoperates with the authenticated local API', async (t) => {
  ...
  const revisedNote = await remote.updateNoteContent(note.id, new TextEncoder().encode('# Revised source note'))
  assert.equal(revisedNote.version, 2)
  await assert.rejects(remote.updateNoteContent(note.id, new TextEncoder().encode('stale'), undefined, 1), error => error.status === 409)
  assert.match(new TextDecoder().decode((await remote.readNote(note.id)).content), /Revised source note/)
  assert.deepEqual((await remote.listNoteVersions(note.id)).map(version => version.version), [2, 1])
  assert.match(new TextDecoder().decode((await remote.readNoteVersion(note.id, 1)).content), /Private source note/)
  const restoredNote = await remote.restoreNoteVersion(note.id, 1, 2)
  assert.equal(restoredNote.version, 3)
  assert.match(new TextDecoder().decode((await remote.readNote(note.id)).content), /Private source note/)
